...
class _RegisterPetScreenState extends ConsumerState<RegisterPetScreen> {
  ...
  Future<void> _loadRegisteredPets() async {
    try {
      ...
      final pets = ref.read(ownerPetsProvider).valueOrNull ?? const [];

      if (!mounted) return;

      setState(() {
        _registeredPets = pets.map<_RegisteredPet>((pet) {
  ...
  Future<void> _onSavePets() async {
    ...
    try {
      ...
      final ownerId = await ref.read(petRepositoryProvider).ownerId();
      if (ownerId == null) {
        throw Exception(
      ...
      for (final pet in _pets) {
        ...
        final result = await ref.read(ownerPetsProvider.notifier).createPet(
              draft: PetDraft.fromMap(
                petData,
                imagePath: pet.selectedImage?.path,
              ),
            );

        createdPetIds.add(result.id);
        if (result.uploadedPath != null) {
          uploadedPaths.add(result.uploadedPath!);
        }
        hasPhotoUploadError = hasPhotoUploadError || result.photoFailed;
      }
      ...
      for (final uploadedPath in uploadedPaths) {
        try {
          await ref.read(petRepositoryProvider).removePhoto(uploadedPath);
        } catch (_) {
      ...
      for (final petId in createdPetIds) {
        try {
          await ref.read(ownerPetsProvider.notifier).deletePet(petId);
        } catch (_) {
          // No ocultamos el error original.
        }
      }
